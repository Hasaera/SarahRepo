...
int main() {
  ...
  waitForLabyrinth("TRAINING DONTMOVE timeout=1000 display=debug  ", monLab, &sizeX, &sizeY);
		...
		if(turn == 0 ){
	 	printLabyrinth();
		 	...
		 	if( tileW>0 ) printf("il y a un mur direction Ouest\n");
	  		...
	  		scanf("%d", &rotation1 );
	  		printf("Entrez le nombre de colonne dont vous voulez vous déplacer  (nombre entre 0 et %d) \n", sizeX);
	  		scanf("%d", &X1);
	  		printf("Entrez le nombre de ligne dont vous voulez vous déplacer(nombre entre 0 et %d) \n", sizeY);
	  		scanf("%d", &Y1);
	  		moveME->insert = tuile;
